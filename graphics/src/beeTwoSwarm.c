...
int main(int argc, char *argv[]) {
	...
	beeSwarm = swarm_create(&start, &velocity, bee, 1, 10, 10);
	...
	for (frame=0; frame<numFrames; frame++){
		...
		// write out image
		sprintf(buffer, "beeTwoSwarm-frame%03d.ppm", frame);
	...
	system("convert -delay 3 -loop 0 beeTwoSwarm-frame*.ppm beeTwoSwarm.gif");
	printf("converted gif\n");
	// remove ppm files
	system("rm beeTwoSwarm-frame*.ppm");
	printf("animating gif...\n");
	// animate gif
	system("animate beeTwoSwarm.gif");
