...
static Module* genFlame(float x, float y, float z, int divisions){
	...
	module_scale(flame, (float)(rand()%10), (float)(rand()%30), (float)(rand()%20));
	module_color(flame, &final);
	module_bezierSurface(flame, &bc, divisions, 1);

	printf("Creating flame with %d subdivisions\n", divisions);
...
int main(int argc, char *argv[]) {
	...
	Image *src;
	Module *woodpile, *fireside, *flame[1];
	Polygon p;
	View3D view;
	Matrix vtm, gtm;
	DrawState *ds;
	...
	matrix_setView3D( &vtm, &view );
 	...
 	// try sizeof(flame[])
 	for ( i = 0; i < 1; i++){
 		flame[i] = module_create();
 		flame[i] = genFlame(0, 0, 0, 5);
 		module_module(fireside, flame[i]);
 	}
	...
	// free the modules
	for ( i = 0; i < sizeof(flame); i++){
		module_delete(flame[i]);
	}
